...
import React from "react";
import { useParams } from "react-router-dom";
import { useProductStore } from "@/stores/productStore"; // Assuming this exists
import { useCartStore } from "@/stores/cartStore"; // This is your updated store
import { Button } from "@/components/ui/button"; // Assuming this is your UI button

export default function ProductDetailsPage() {
  const { id } = useParams();
  const products = useProductStore((state) => state.products);
  const addToCart = useCartStore((state) => state.addToCart);
  // Get loading and error states for UI feedback
  const isAddingToCart = useCartStore((state) => state.isAddingToCart);
  const addToCartError = useCartStore((state) => state.addToCartError);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-2 gap-10">
      ...
      <div>
        ...
        {addToCartError && (
          <p className="text-red-500 mt-2">Error: {addToCartError}</p>
        )}
      </div>
    </div>
  );
